    ...
    <div className="space-y-6">
      ...
      {activeTab === "config" && (
        <div className="max-w-2xl space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-sm placeholder:text-white/30 focus:outline-none focus:border-white/20"
                />
              </div>
              <div>
          ...
          {/* AI Behavior */}
          <div className="rounded-xl bg-white/[0.03] border border-white/[0.08] p-5">
            <h3 className="text-sm font-semibold mb-4">AI Behavior</h3>

            {/* Tone */}
            <div className="mb-5">
              <label className="block text-xs text-white/50 mb-2">Conversation Tone</label>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03]">
                ...
                <button
                  onClick={() => setSettings({ ...settings, auto_respond: !settings.auto_respond })}
                  className={`w-10 h-5 rounded-full transition-all relative ${settings.auto_respond ? "bg-white" : "bg-white/20"}`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${settings.auto_respond ? "translate-x-[22px] bg-black" : "translate-x-0.5"}`} />
                </button>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03]">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="w-12 px-2 py-1 rounded bg-white/[0.05] border border-white/[0.08] text-sm text-center focus:outline-none"
                  />
              ...
              <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`w-10 h-5 rounded-full transition-all relative ${settings.require_deposit ? "bg-white" : "bg-white/20"}`}
                  >
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${settings.require_deposit ? "translate-x-[22px] bg-black" : "translate-x-0.5"}`} />
                  </button>
                  {settings.require_deposit && (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="w-10 px-1 py-1 rounded bg-white/[0.05] border border-white/[0.08] text-sm text-center focus:outline-none"
                      />
              ...
              <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`w-10 h-5 rounded-full transition-all relative ${settings.follow_up_enabled ? "bg-white" : "bg-white/20"}`}
                  >
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${settings.follow_up_enabled ? "translate-x-[22px] bg-black" : "translate-x-0.5"}`} />
                  </button>
          ...
          {/* Messages */}
          <div className="rounded-xl bg-white/[0.03] border border-white/[0.08] p-5">
            <h3 className="text-sm font-semibold mb-4">Default Messages</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-white/50 mb-1.5">Greeting</label>
                <textarea
                  rows={2}
                  placeholder="Hey! Thanks for reaching out..."
                  value={settings.greeting_message}
                  onChange={(e) => setSettings({ ...settings, greeting_message: e.target.value })}
                  className="w-full px-3 py-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-sm placeholder:text-white/30 focus:outline-none focus:border-white/20 resize-none"
                />
              </div>
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="To secure your booking, we require..."
          ...
          {/* Advanced */}
          <details className="rounded-xl bg-white/[0.03] border border-white/[0.08] overflow-hidden">
            <summary className="p-5 cursor-pointer flex items-center justify-between hover:bg-white/[0.02] transition-colors">
              <div>
                <h3 className="text-sm font-semibold">Custom System Prompt</h3>
                <p className="text-xs text-white/40 mt-0.5">Override the default AI instructions</p>
              </div>
            ...
            <div className="px-5 pb-5 space-y-3 border-t border-white/[0.06] pt-4">
              <div className="flex justify-between items-center">
                <p className="text-xs text-white/40">Leave empty to use defaults</p>
                <button
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-sm font-mono placeholder:text-white/15 focus:outline-none focus:border-white/20 resize-none"
              />
              <div className="flex flex-wrap gap-1.5">
                {["{{business_name}}", "{{tone}}", "{{vehicles}}"].map((v) => (
                  <button
                    key={v}
                    onClick={() => navigator.clipboard.writeText(v)}
                    className="px-2 py-0.5 rounded bg-white/[0.05] text-[10px] font-mono text-white/50 hover:text-white"
                  >
                    {v}
                  </button>
                ))}
          ...
          <div className="flex gap-4 text-sm">
            ...
            <div className="flex items-center gap-2 text-white/50">
              <Zap className={settings.auto_respond ? "w-4 h-4 text-white" : "w-4 h-4"} />
              <span>{settings.auto_respond ? "AI active" : "AI paused"}</span>
            </div>
          </div>
        </div>
      )}
